...
ClapTrap::ClapTrap(ClapTrap const & other) : 
		...
		max_energy_points_(other.max_energy_points()),
		level_(other.level()),
		name_(other.name()),
...
unsigned int	ClapTrap::max_energy_points() const			{ return max_energy_points_; }
...
void			ClapTrap::set_max_energy_points(unsigned int num)		{ max_energy_points_ = num; }
...
std::string		ClapTrap::get_health() const
{
	std::stringstream	temp;

	temp << BLU << "<" << "CL4P-TP: " << name() << " "
		 << hit_points() << "/"
		 << max_hit_points() << "> " << RESET;
	return temp.str();
}

unsigned int	ClapTrap::take_damage_list_len = 6;


const char	*ClapTrap::take_damage_list[] = {
...
unsigned int	ClapTrap::be_repaired_list_len = 6;

const char	*ClapTrap::be_repaired_list[] = {
